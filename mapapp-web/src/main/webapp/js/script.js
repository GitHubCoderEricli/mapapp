...
var mapWin ={
	...
	"goto":function (id,pointsData){  //弹窗按钮初始化
		...
		btn.addEventListener('touchstart',function(){
			var nowPoint = new BMap.Point(110.031241,34.50719);  // 创建点坐标
...
var mapNavigation = {
	...
	"showPath":function (startPoint, EndPoint,num) {
		...
		walking.setSearchCompleteCallback(function (rs) {
			...
			var line1 = new BMap.Polyline(pts, { strokeColor: "green", strokeWeight: 2, strokeOpacity: 1 })
			map.addOverlay(line1);
			mapNavigationMarkerArray.push(line1);
		});
		walking.disableAutoViewport();
		walking.setMarkersSetCallback(function (array) {
			var point0 = array[0].marker;
		...
		map.addOverlay(m1);
		mapNavigationMarkerArray.push(m1);
		...
		map.addOverlay(lab1);
		mapNavigationMarkerArray.push(lab1);
	}
}
...
var map = new BMap.Map("container",{minZoom:4,maxZoom:18});          // 创建地图实例
...
/*map.addEventListener("dragend", function(){
 ...
 // map.addEventListener("click", function(e){
	//  alert(e.point.longitude + ", " + e.point.latitude);
 // });