...
public class EscColumnToBean implements ResultTransformer {
    ...
    public Object transformTuple(Object[] tuple, String[] aliases) {
        ...
        try {
            ...
            for (int i = 0; i < aliases.length; i++) {
                if(setters[i]!=null) {
                    if(setters[i].getMethod().getParameterTypes()[0].getName().endsWith("Integer")&&tuple[i].getClass().getTypeName().endsWith("Byte")){
                        setters[i].set(result, Integer.valueOf((Byte) tuple[i]), null);
                    }else
