...
!function ($) {
    ...
    BootstrapTable.prototype.initToolbar = function () {
            ...
            $.each(this.options.columns, function (i, column) {
                ...
                if (column.switchable) {
                    html.push(sprintf('<li>' +
                        '<label><table><tr><td><input type="checkbox" data-field="%s" value="%s"%s></td><td> %s</td></tr></table></label>' +
        ...
        if (this.options.search) {
            ...
            $search.off('keyup').on('keyup', function (event) {
                // *** modified by devonline.cn change keyup to only the "Entry" key up to trigger
                if (event.keyCode === 13) {
    ...
    BootstrapTable.prototype.initServer = function (silent, query) {
        ...
        // *** modified by devonline.cn add bellow codes to change filter expression to oData standard
        params.filter = this.options.filter || "true";
        params.filter += this.options.queryExpression ? " and " + this.options.queryExpression : "";
        if (this.options.search && params.search) {
            var columns = $.com.where(this.options.columns, $.com.filter("searchable", true));
            var filters = [];
            $(columns).each(function () {
                filters.push({ field: this.field, value: params.search, logic: "co" });
            });
            params.filter += " and " + $.mvc.getFilterExpression(filters, "or");
        }

        //if (!$.com.isNullOrEmpty(this.filterColumnsPartial)) {
        //    var filters = [];
        //    $.each(this.filterColumnsPartial, function (i, column) {
        //        filters.push({ field: column.field, value: column.text, logic: "co" });
        //    });
        //    params.filter += " and " + $.mvc.getFilterExpression(filters, "or");
        //}
        // *** end
